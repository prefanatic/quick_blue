...
class QuickBlueAndroid extends QuickBluePlatform {
  ...
  Future<BleRangingCapabilities> rangingCapabilities(String deviceId) async {
    _ensureInitialized();
    final capabilities = await _runRangingOperation(
...
Future<T> _runRangingOperation<T>({
  required String operation,
  required String deviceId,
  required Future<T> Function() action,
}) async {
...
QuickBlueException _rangingException({
  required String operation,
  required String deviceId,
  ...
  final code = switch (platformCode) {
    ...
    'Unavailable' || 'PermissionDenied' => QuickBlueErrorCode.unavailable,
